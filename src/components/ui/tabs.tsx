...
const TabsList = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(
  ({ className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "inline-flex h-11 items-center justify-center rounded-full bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl border border-white/80 dark:border-white/20 shadow-md shadow-slate-900/5 p-1 text-muted-foreground w-full sm:w-auto gap-1 relative z-10",
        className
      )}
      {...props}
    />
      ...
      <button
        ...
        className={cn(
          ...
          isSelected
            ...
            : "text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-white hover:bg-slate-100/80 dark:hover:bg-slate-800/80 font-extrabold",
          className
        )}
        {...props}
      />
